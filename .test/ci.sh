...
for ver in 0.4 0.5 0.6 nightly; do
  if [ $ver = "nightly" ]; then
    url="https://julialangnightlies-s3.julialang.org/bin/linux/x64/julia-latest-linux64"
    ver=0.7
  else
    url="https://julialang-s3.julialang.org/bin/linux/x64/$ver/julia-$ver-latest-linux-x86_64"
  fi
